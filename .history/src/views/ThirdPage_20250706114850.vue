...
const cards = [
  {
    icon: "🚀",
    title: "最有活力的技术组织",
    desc: "我们是 SMBUCA，在这里你可以写代码、打比赛、做项目，也可以一起搞事！",
  },
  {
    icon: "🎯",
    title: "什么都敢试，什么都能聊",
    desc: "AI、前端、算法、逆向……每一次分享都是新的技术冒险。",
  },
  {
    icon: "🧩",
    title: "不限专业，不限年级",
    desc: "只要你对计算机感兴趣，这里总有属于你的一块拼图。",
  },
  {
    icon: "🌏",
    title: "认真写代码，也认真玩",
    desc: "我们组织聚会、联动活动、摸鱼交流群，在学习之外也收获快乐。",
  },
]
...
onMounted(() => {
  ...
  // 自动横向轮播
  let scrollEl = carousel.value
  let scrollAmount = 0
  const scrollSpeed = 1 // px per frame
  const maxScroll = scrollEl.scrollWidth

  function autoScroll() {
    if (scrollEl.scrollLeft + scrollEl.offsetWidth >= scrollEl.scrollWidth) {
      scrollEl.scrollTo({ left: 0, behavior: 'auto' }) // loop
    } else {
